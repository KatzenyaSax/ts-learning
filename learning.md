...
ts 是编译型语言，但是与传统编译型语言这个概念大不相同：传统的编译语言会将高级语言源代码编译成硬件可执行的目标代码，拿到可运行的目标代码后，源代码就不需要再编译了；而 ts 的 "编译" 则单单指的是将 ts 转化为 js，但是以 js 的方式实现 ts 中的一些强制规范，从而达成间接为 js 上强制规范的效果。
